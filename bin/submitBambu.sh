...
then
  ...
  tar fzc default.tgz bin/  tgz/copy.tgz tgz/siteconf.tgz \
          $VERSION/${TASK}* $VERSION/bambu* $VERSION/python/Bambu* $VERSION/tgz/bambu${VERSION}.tgz
...
Notification            = Error
Executable              = $script
Arguments               = $VERSION $TASK $gpack $CRAB
Rank                    = Mips
GetEnv                  = False
Input                   = /dev/null
