...
void makeplot(const char *name, TTree *tree, TCut weight, const char *drawstring,
	      const char *xlabel, int nbins, double xlow, double xhigh, int typeMC)
{
  ...
  // I want to have all the histograms
  hall->Write();
  hmult0->Write();
  hmult1->Write();
  hmult2->Write();
...
void plotQCut(const TString & infile, const TString & outfile)
{
